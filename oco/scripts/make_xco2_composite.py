#!/usr/bin/env python3
"""Per-cell MEDIAN of OCO-2 L2 Lite FP (v11.2r) good-quality XCO2 (land and ocean glint) on a regular grid,
for a set of months over several years — one season, so summer-only high-latitude sampling does not bias the map.
Usage: make_xco2_composite.py <out.npz> <res_deg> <years e.g. 2021-2024> <months e.g. 6,7,8>"""
import glob, sys, numpy as np, netCDF4 as nc
out, res = sys.argv[1], float(sys.argv[2]); y0, y1 = [int(v) for v in sys.argv[3].split("-")]; months = [int(m) for m in sys.argv[4].split(",")]
files = [f for y in range(y0, y1 + 1) for f in sorted(glob.glob(f"/kiwi-data/Data/satellite/OCO2/L2_Lite_FP/11.2r/raw/{y}/oco2_LtCO2_*.nc4")) if int(f.split("_LtCO2_")[1][2:4]) in months]
...
np.savez(out, median=med.reshape(ny, nx), N=N.reshape(ny, nx), res=res, years=f"{y0}-{y1}", months=months, n_files=nf)
